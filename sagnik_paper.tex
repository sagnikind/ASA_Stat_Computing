...
%random field. Another popular technique to model 
...
%
%Several studies have tackled the computational challenges of large-scale geospatial datasets in literature through parallel processing. For example, the authors in \cite{2017.T.L.R.S.G.G.L.G.T.M.L.R.D.S.JournalofComputationalInterdisciplinarySciences},  \cite{2013.T.C.ComputersandGeosciences}, \cite{2012.P.T.M.S.G.M.A.H.ComputersandGeosciences} have implemented kriging for Gaussian model in parallel 
...
of a $8100 \times 8100$ covariance matrix generated by a
...
\begin{subfigure}{.178\textwidth}
  ...
  \includegraphics[width=\linewidth]{{./figures/log_matern_0.7_0.5_0_2_0.2_0.2_8100_810_lr_9}.pdf}
   \caption{ $10^{-9}$.}
\end{subfigure}
\caption{Rank distributions of a $8100 \times 8100$ non-Gaussian covariance - nb = 810 with TGH Mat\'ern parameters $\bm{\theta} = (0.96, 0.5, 0, 2, 0.5, 0.3)^\top$ under different accuracy levels.}
...
%low-rank structure using TLR approximation.
%The baseline TLR software is the HiCMA library, which runs on
%both shared-memory and distributed memory architectures. We compare the TGH non-Gaussian model using TLR
...
%Furthermore, Figure~\ref{fig:tlr-exact-pred} shows the performance of the prediction operation using
